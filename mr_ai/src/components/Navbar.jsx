...
const Navbar = () => {
  ...
  return (
    ...
    {
        user ? <UserButton /> : (
        <button onClick={() => openSignIn()} className='flex items-center gap-2 rounded-full bg-primary text-sm cursor-pointer px-10 py-2.5'>Get Started <ArrowRight className='w-4 h-4'/> </button>
      )
    }
    </div>
  )
